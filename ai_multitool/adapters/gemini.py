"""Gemini CLI adapter for ai-multitool integration."""
...
class GeminiAdapter(BaseAdapter):
    """Adapter for integrating ai-multitool with Gemini CLI.

    This adapter provides tools in Gemini's expected format and
    handles authentication through Gemini's credential system.
    """
    ...
    def _create_llm_client(self) -> AnthropicClient:
        """Create LLM client for Gemini.

        Gemini can use either Anthropic or OpenAI.
        ...
        if self.config.provider == Provider.ANTHROPIC:
            ...
            raise ValueError(f"Unsupported provider for Gemini: {self.config.provider}")

    def get_tool_definitions(self) -> List[Dict[str, Any]]:
        """Return tool definitions in Gemini format.

        Gemini uses OpenAI's function format.
    ...
    def execute_gemini_command(self, command: str, **kwargs) -> Any:
        """Execute a Gemini-specific command.
        ...
        Returns:
            Command result
        """
        # This can be extended with Gemini-specific commands
...
# Example usage for Gemini integration
